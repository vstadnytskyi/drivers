...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=31, verbose=False, dllname='APT.dll'):
        ...
        self.aptdll = cdll.LoadLibrary(dllname)
        ...
        if SerialNum is not None:
            if self.verbose: print("Serial is %r" % SerialNum)
    ...
    def setSerialNumber(self, SerialNum):
        ...
        '''
        if self.verbose: print("Serial is %s" %SerialNum)
        self.SerialNum = c_long(SerialNum)
    ...
    def initializeHardwareDevice(self):
        ...
        '''
        if self.verbose: print('initializeHardwareDevice serial %s' %self.SerialNum)
    ...
    def setVel(self, maxVel):
        if self.verbose: print('setVel %s' %maxVel)
    ...
    def getPos(self):
        ...
        if self.verbose: print('getPos %s' %position.value)
    ...
    def mRel(self, relDistance):
        ...
        '''
        if self.verbose: print('mRel %s,%s' %(relDistance, c_float(relDistance)))
        if not self.Connected:
    ...
    def mAbs(self, absPosition):
        ...
        '''
        if self.verbose: print('mAbs %s, %s' %(absPosition, c_float(absPosition)))
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
    ...
    def mcRel(self, relDistance, moveVel=0.5):
        ...
        '''
        if self.verbose: print('mcRel %s,%s mVel %s' %( relDistance, c_float(relDistance), moveVel))
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
    ...
    def mcAbs(self, absPosition, moveVel=0.5):
        ...
        moveVel        float     Motor velocity, mm/sec
        '''
        if self.verbose: print('mcAbs%s %s , mVel %s' %(absPosition, c_float(absPosition),  moveVel))
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
        # Save velocities to reset after move
        minVel, acc, maxVel = self.getVelocityParameters()
    ...
    def mbRel(self, relDistance):
        ...
        relDistance    float     Relative position desired
        '''
        if self.verbose: print('mbRel %s %s' %( relDistance, c_float(relDistance)))
        if not self.Connected:
            print('Please connect first! Use initializeHardwareDevice')
            #raise Exception('Please connect first! Use initializeHardwareDevice')
    ...
    def mbAbs(self, absPosition):
        ...
        absPosition    float     Position desired
        '''
        if self.verbose: print('mbAbs %s, %s' %(absPosition, c_float(absPosition)))
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
        if (absPosition < self.getPos()):
            if self.verbose: print('backlash mAbs %s' %(absPosition - self.blCorr))
