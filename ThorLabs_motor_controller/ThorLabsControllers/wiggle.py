...
#dev0 = APTMotor(SerialNum = 83836040,verbose = True)

dev0 = APTMotor(SerialNum = 83837769,verbose = True)
time_step = 0.6
for i in range(1):
    print('move',i)
    position = c_float()
    dev0.aptdll.MOT_GetPosition(dev0.SerialNum, pointer(position))
    print('position', position.value)
    dev0.aptdll.MOT_MoveVelocity(dev0.SerialNum,1)
    sleep(time_step)
    dev0.aptdll.MOT_MoveVelocity(dev0.SerialNum,2)
    sleep(time_step)
    dev0.aptdll.MOT_MoveVelocity(dev0.SerialNum,1)
    sleep(time_step)
    dev0.aptdll.MOT_MoveVelocity(dev0.SerialNum,2)
