...
class Cube(object):
    def __init__(self, port = None, controller = None, motor = None):
        """controllers can be T or K - cubes. This has to be specified because
        different controllers have different sequence of bytes for the same command"""
        if port == None:
            raise ValueError('The COM port has not been specified')
        else:
            self.port = port
            self.ser = Serial(port, baudrate=115200, bytesize = 8, parity='N', stopbits=1, timeout=1)
    ...
    def discover(self):
        ...
        for element in lst:
            ...
            if element.description.find('DI245') > -1:
                ...
                self.available_ports.append([element.device,element.serial_number])
    ...
    def identify(self):
        ...
        Header = result[0:6]
        SerialNumber = int(unpack('i',result[6:10])) #unpack('L',result[6:10])
